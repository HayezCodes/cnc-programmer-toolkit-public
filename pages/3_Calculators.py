...
from data.general_references import DECIMAL_FRACTION_ROWS
...
Choose a category to work directly in the matching shop math calculators.
</div>
""",
    unsafe_allow_html=True,
)
...
with section_holemaking:
    chamfer_expander, center_expander, breakthrough_expander = st.columns(3)

    with chamfer_expander:
        with st.expander("Spot Drill & Hole Chamfer", expanded=True):
            ...
            included_angle = st.number_input(
                ...
                step=1.0,
                format="%.1f",
                key="workbench_chamfer_angle",
            )
            try:
                chamfer_depth = chamfer_depth_from_diameters(pilot_dia, target_od, included_angle)
                chamfer_width = chamfer_width_from_depth(chamfer_depth, included_angle)
            except ValueError as exc:
                st.error(str(exc))
            else:
                st.metric("Axial Depth", format_shop_decimal(chamfer_depth))
                st.caption(f"Width / radial growth: {format_shop_decimal(chamfer_width)}")
    with center_expander:
        with st.expander("Center Drill", expanded=True):
            selected_size = st.selectbox(
                "Size",
                get_center_drill_options(include_custom=False),
                format_func=center_drill_label,
                key="workbench_center_drill_size",
            )
            center_drill_data = CENTER_DRILL_PRESETS[selected_size]
            target_spot_dia = st.number_input(
                "Target Spot Dia",
                min_value=0.0001,
                value=max(center_drill_data["pilot"] + 0.1250, 0.2500),
                step=0.0010,
                format="%.4f",
                key="workbench_center_drill_target",
            )
            try:
                center_depth = center_drill_total_depth_for_target(
                    center_drill_data["pilot"],
                    target_spot_dia,
                    center_drill_data["angle"],
                    center_drill_data["pilot_length"],
                )
            except ValueError as exc:
                st.error(str(exc))
            else:
                st.metric("Depth From Face", format_shop_decimal(center_depth))
                st.caption(
                    f"Pilot {format_shop_decimal(center_drill_data['pilot'])}, "
                    f"angle {center_drill_data['angle']:.0f} deg"
                )
    with breakthrough_expander:
        with st.expander("Drill Breakthrough", expanded=True):
            drill_dia = st.number_input(
                "Drill Dia",
                min_value=0.0001,
                value=0.2500,
                step=0.0010,
                format="%.4f",
                key="workbench_breakthrough_dia",
            )
            drill_point_angle = st.number_input(
                "Point Angle",
                min_value=1.0,
                max_value=179.0,
                value=118.0,
                step=1.0,
                format="%.1f",
                key="workbench_breakthrough_angle",
            )
            past_center = st.number_input(
                "Past Center",
                min_value=0.0000,
                value=0.0200,
                step=0.0010,
                format="%.4f",
                key="workbench_breakthrough_past",
            )
            point_height = (drill_dia / 2) / math.tan(math.radians(drill_point_angle / 2))
            st.metric("Additional Depth", format_shop_decimal(point_height + past_center))
            st.caption("Depth is referenced from the point first reaching the far side.")

with section_geometry:
    geometry_triangle, geometry_keyway, geometry_woodruff, geometry_slot_chamfer = st.tabs(
        ["Triangle", "Keyway / Slot", "Woodruff Key", "Slot Chamfer"]
    )

    with geometry_triangle:
    ...
    **Reference**
    ...
    with geometry_slot_chamfer:
        st.subheader("Keyway / Slot Chamfer Calculator")
    
        st.markdown(
            """
    <div style="font-size:0.92rem; line-height:1.35;">
    Use this for keyway and slot edge-break math. Spot drill and hole chamfer OD depth is in the Holemaking Math tab.
    </div>
    """,
            unsafe_allow_html=True,
        )
...
with section_conversions:
    conversion_live, conversion_fraction = st.tabs(["IN ↔ MM Conversion", "Decimal / Fraction"] )

    with conversion_live:
    ...
    with conversion_fraction:
        st.markdown("### Decimal / Fraction Reference")
        st.dataframe(DECIMAL_FRACTION_ROWS, use_container_width=True, hide_index=True)
